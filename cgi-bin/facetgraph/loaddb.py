...
def createCameraTopicRI():
	...
	N = 500
	# count all the topics and cameras
	f = open(INPUT_FILE)
	f.readline()
	l = f.readline()
	i = 0
	while len(l) > 0:
		...
		l = f.readline()
		if True:
			cameraName = line[1].split("/")[-1].split(".")[0].replace("_camera", "").replace("_", " ").replace("-", " ")
			if cameraName in index:
				id = getCameraID(cameraName)
				print "%d %s"%(i, cameraName)
				j = 3;
				queue = util.PriorityQueue()
				ct = [0 for n in xrange(N)]
				while j < len(line):
					topic = int(line[j-1])
					if topic not in tc:
						tc [topic] = util.Counter()
						tcq[topic] = util.PriorityQueue()
					proportion = float(line[j])
					ct[topic] = proportion
					queue.push(topic, proportion)
					tc[topic][id] = proportion
					tcq[topic].push(id, proportion)
					j += 2
				rank = 0
				while not queue.isEmpty():
					topic = queue.pop()
					score = ct[topic]
					instance = RI(id, "camera", topic, "topic", rank, score)
					#print instance
					session.add(instance)
					rank +=1
				session.commit()
	...
	for topic in tcq:
		...
		while not queue.isEmpty():
			cameraID = queue.pop()
			score = tc[topic][camera]
			instance = RI(topic, "topic", cameraID, "camera", rank, score)
			#print instance
			session.add(instance)
			rank += 1
		session.commit()
